...
export default function BotsPage() {
    ...
    const handleGenerate = async () => {
        ...
        const prompt = `Create a comprehensive system prompt for an AI bot with the following details:\n\nName: ${name}\nRole/Key Responsibilities: ${role}\n\nThe system prompt should define its personality, constraints, interaction style, and expertise.`
        ...
        <div className="min-h-screen bg-white dark:bg-zinc-950">
            <header className="h-14 px-4 flex items-center justify-between border-b border-gray-100 dark:border-zinc-800">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">Bot Creator</h1>
                </div>
            </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. CodeWizard, FitCoach..."
                                className="w-full px-4 py-2 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-4"
                            />

                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Role & Capabilities</label>
                            <textarea
                                value={role}
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Generating...</> : 'Generate System Prompt'}
                            </button>
                            <button onClick={() => { setName(''); setRole(''); setOutput('') }} className="px-4 py-3 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-xl transition-colors">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Generated Prompt</label>
                            {output && (
                        ...
                        <div className="min-h-[400px] p-6 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl font-mono text-sm whitespace-pre-wrap">
                            {loading ? (
                                <div className="flex items-center justify-center h-full"><Loader2 className="w-8 h-8 animate-spin text-indigo-500" /></div>
                            ) : output ? (
                                output
                            ) : (
                                <p className="text-gray-400 dark:text-gray-500">System prompt will appear here</p>
                            )}
                        </div>
                    </div>
                </div>
