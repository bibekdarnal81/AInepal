...
export default function HumanizerPage() {
    ...
    const handleGenerate = async () => {
        ...
        const prompt = `Rewrite the following text to sound more human, natural, and less robotic. Avoid jargon and overly complex sentence structures. \n\nText to humanize:\n"${text}"`
        ...
        try {
            ...
            if (!response.ok) throw new Error(data.error || 'Failed to humanize')
        ...
        <div className="min-h-screen bg-white dark:bg-zinc-950">
            <header className="h-14 px-4 flex items-center justify-between border-b border-gray-100 dark:border-zinc-800">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">AI Humanizer</h1>
                </div>
            </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-6">
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Humanizing...</> : 'Humanize Text'}
                            </button>
                            <button onClick={() => { setText(''); setOutput('') }} className="px-4 py-3 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-xl transition-colors">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Humanized Result</label>
                            {output && (
                        ...
                        <div className="min-h-[400px] p-6 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl">
                            {loading ? (
                                ...
                                <p className="text-gray-400 dark:text-gray-500">Humanized text will appear here</p>
                            )}
                        </div>
                    </div>
                </div>
